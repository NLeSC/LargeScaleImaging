% dmssr_open- function to open the saved results from the DMSR detector
%**************************************************************************
% [num_regions, features, saliency_masks] = dmsr_open(features_fname,...
%                                                     regions_fname, type)
%
...
%**************************************************************************
% INPUTS:
% features_fname- the filename for the ellipse features (.dmsr)
...
%**************************************************************************
% NOTES: the opposite of dmsr_save
%**************************************************************************
...
function  [num_regions, features, saliency_masks] =...
    dmsr_open(features_fname, regions_fname, type) 
...
elseif nargin < 2
    regions_fname = [];
elseif nargin < 1
    error('dmsr_open.m requires at least 1 input argument!');
    return
end
%**************************************************************************
...
if type
    ...
    j = i(end);
    if isempty(j)
        features_type_fname = [features_fname '_type.dmsr'];
    else
        features_type_fname = [features_fname(1:j-1) '_type.dmsr'];
    end
end
%**************************************************************************
% initialisations
