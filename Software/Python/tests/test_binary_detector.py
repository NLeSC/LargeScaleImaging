...
class BinaryDetectorTester(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.image_noise = sr.binarize(cv2.imread(os.path.join(testdata_path, 'Binary_all_types_noise.png')), threshold=128, visualize=False)
        self.image_nested = sr.binarize(cv2.imread(os.path.join(testdata_path, 'Binary_nested.png')), threshold=128, visualize=False)
