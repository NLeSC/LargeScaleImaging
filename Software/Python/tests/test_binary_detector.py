...
class BinaryDetectorTester(unittest.TestCase):
    ...
    def test_holes(self):
        _, holes_my = sr.get_holes(self.image, filled=None, self.lam=lam, 
    ...
    def test_protrusions(self):
        _, prots_my = sr.get_protrusions(self.image, filled=None, holes=None, SE=self.SE, 
                                         lam=self.lam, area_factor=self.area_factor, 
                                         connectivity=4, vizualize=false)
        assert sr.image_diff(self.prots_true, prots_my, vizualize=False)
        
    def test_indentations(self):
        _, indents_my = sr.get_indentations(self.image, invfilled=None, islands=None, SE=self.SE, 
                                            lam=self.lam, area_factor=self.area_factor, 
                                            connectivity=4, vizualize=false)
