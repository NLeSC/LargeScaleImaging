...
def get_holes(img, filled=None, lam=-1, connectivity=4, visualize=True):
    ...
    theholes = remove_small_elements(all_the_holes,
                                     lam=lam,
...
def get_islands(img,  invfilled=None, lam=-1, connectivity=4, visualize=True):
    ...
    invfilled, islands = get_holes(invimg, filled=invfilled,
                                   lam=lam, connectivity=connectivity,
                                   visualize=visualize)
    return invfilled, islands


def remove_small_elements(elements, lam, remove_border_elements=True,
...
def get_protrusions(img,
                    filled=None, holes=None,
                    SE=None, lam=-1, area_factor=0.05,
                    connectivity=4, visualize=True):
    '''
    ...
    '''
    if holes is None:
        filled, holes = get_holes(img, filled=filled, lam=lam,
                                  connectivity=connectivity, visualize=False)
    ...
    prots1_nonoise = remove_small_elements(prots1, lam,
                                           connectivity=8,
                                           visualize=visualize)
    ...
    prots2_nonoise = remove_small_elements(prots2, lam, connectivity=8,
                                           visualize=visualize)
...
def get_indentations(img,
                     invfilled=None, islands=None,
                     SE=None, lam=-1, area_factor=0.05,
                     connectivity=4, visualize=True):
    '''
    Find salient regions of type 'island'
    ...
    invimg = cv2.bitwise_not(img)
    if islands is None:
        invfilled, islands = get_islands(img,
                                         invfilled=invfilled, lam=lam,
                                         connectivity=connectivity,
                                         visualize=False)
    if invfilled is None:
        invfilled = fill_image(invimg, visualize=visualize)
    invfilled, indentations = get_protrusions(invimg,
                                              filled=invfilled, holes=islands,
                                              SE=SE, lam=lam,
...
def get_salient_regions_binary(img,
    ...
    if find_holes or find_protrusions:
        filled, holes = get_holes(img, filled=None, lam=lam,
                                  connectivity=connectivity,
                                  visualize=visualize)
        if find_holes:
            regions['holes'] = holes
    ...
    if find_indentations:
        invfilled, indentations = get_indentations(img,
                                                   invfilled=invfilled,
                                                   islands=islands,
                                                   SE=SE, lam=lam,
                                                   area_factor=area_factor,
    ...
    if find_protrusions:
        filled, protrusions = get_protrusions(img,
                                              filled=filled,
                                              holes=holes,
                                              SE=SE, lam=lam,
                                              area_factor=area_factor,
